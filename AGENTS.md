...
- **No `permissions` key in the manifest.** The on-device model API needs no manifest permission per current Chrome docs, and several Chrome builds warn on the unknown `aiLanguageModel` permission string — so none is declared; the only grant is `host_permissions` for api.typesafe.ai. Detection happens at runtime (`typeof` guards); do not re-add `activeTab`/`scripting`/`aiLanguageModel` without a concrete code path and verified browser support.
